...
function sink(heap, parentIndex) {
  ...
  // Guard against second child possibly being undefined
  let smallestChildIndex;
  if (childIndexB >= heap.length) { smallestChildIndex = childIndexA; }
  else {
    smallestChildIndex = heap[childIndexA].key > heap[childIndexB].key
    ? childIndexB
    : childIndexA;
  }
  ...
  if (heap[smallestChildIndex].key > heap[parentIndex].key) { return; }
...
function swim(heap, childIndex) {
  ...
  if (heap[childIndex].key >= heap[parentIndex].key) { return; }

  swap(heap, childIndex, parentIndex);
  swim(heap, parentIndex);
}

/** Class representing a priority queue symbol table */
class MinPriorityQueueTable {
  /**
  ...
  /**
   * @description Remove smallest key in heap and readjust heap to maintain
   ...
   * Space complexity: O(1)
   * 
   * @param {Number|String} key - key used for heap position comparisons
   * @param {*} value - value associated with input key
   */
  insert(key, value) {
    if (isNaN(key) || (typeof key !== 'string' && typeof key !== 'number')) {
      throw new Error('Please insert valid number or string character for key');
    }
...
module.exports = MinPriorityQueueTable;